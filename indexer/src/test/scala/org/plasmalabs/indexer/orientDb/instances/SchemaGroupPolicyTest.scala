...
class SchemaGroupPolicyTest
  ...
  orientDbFixtureNew.test("GroupPolicy Schema Metadata") { case (odbFactory, implicit0(oThread: OrientThread[F])) =>
    val res = for {
  ...
  orientDbFixtureNew.test("GroupPolicy Schema Add vertex") { case (odbFactory, implicit0(oThread: OrientThread[F])) =>
    val res = for {
